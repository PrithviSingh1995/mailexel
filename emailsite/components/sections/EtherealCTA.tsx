...
import { defaultHomeContent, type EtherealCtaContent } from "@/lib/types/page-content";

export default function EtherealCTA({ content }: { content?: Partial<EtherealCtaContent> }) {
  const c: EtherealCtaContent = { ...defaultHomeContent.etherealCta, ...content };
  ...
  return (
    <section className="relative min-h-[600px] w-full flex items-center justify-center overflow-hidden bg-black">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
        ...
        <p className="mb-10 text-lg sm:text-xl leading-relaxed text-white/60 max-w-2xl mx-auto">{c.description}</p>

        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-12">
          <a href={c.primaryCtaHref} className="group relative overflow-hidden rounded-full bg-red-600 text-white font-semibold px-8 py-4 text-base shadow-2xl shadow-red-900/40 hover:bg-red-700 transition-all flex items-center gap-2">
          ...
          <a href={c.secondaryCtaHref} className="rounded-full border border-white/20 bg-white/5 text-white font-semibold px-8 py-4 text-base hover:bg-white/10 hover:border-white/30 transition-all">
            {c.secondaryCtaText}
          </a>
        </div>
          ...
          {c.stats.map((s, i) => (
            <div key={i} className="text-center">
              <div className="flex items-center justify-center gap-1 mb-1">
                ...
                {i === 2 && <Star className="w-5 h-5 text-white fill-white mt-0.5" />}
              </div>
