...
import { FolderOpen, Settings2, Send, CheckCircle2, LucideIcon } from "lucide-react";
import { defaultHomeContent, type HowItWorksContent } from "@/lib/types/page-content";

const stepIcons: LucideIcon[] = [FolderOpen, Settings2, Send];
...
export default function HowItWorksSection({ content }: { content?: Partial<HowItWorksContent> }) {
  const c: HowItWorksContent = { ...defaultHomeContent.howItWorks, ...content };
    ...
    <section id="how-it-works" className="py-24 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-gray-500 max-w-xl mx-auto">{c.description}</p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          <div className="space-y-4">
            {c.steps.map((step, i) => {
              const Icon = stepIcons[i] ?? FolderOpen;
              ...
              return (
                <motion.button
                  ...
                  className={`w-full text-left p-6 rounded-2xl border-2 transition-all duration-300 ${
                    isActive ? "border-black bg-white shadow-xl shadow-black/5" : "border-gray-200 bg-white/70 hover:border-gray-300 hover:shadow-md"
                  }`}
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <div className="text-xs font-black text-red-400 tracking-widest mb-1">STEP {num}</div>
            ...
            <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/80 overflow-hidden border-2 border-gray-100">
              <div className="flex items-center gap-1.5 px-4 py-3 bg-black border-b border-white/10">
                ...
                <div className="flex-1 mx-3 h-5 bg-white/10 rounded text-[10px] flex items-center px-2 text-white/50">
                  MailExel Pro — Step {activeStep + 1}/{c.steps.length}
                </div>
              </div>
              <div className="flex">
                {c.steps.map((_, i) => (
              ...
              <div className="px-5 py-3 bg-gray-50 border-t border-gray-100 flex justify-between items-center">
                ...
                <button onClick={() => setActiveStep(Math.min(c.steps.length - 1, activeStep + 1))} disabled={activeStep === c.steps.length - 1} className="text-xs text-red-600 font-semibold disabled:opacity-30">Next →</button>
              </div>
            </div>
          </div>
        </div>
