...
/**
 * Adds IDs to headings for TOC anchors.
 * When content uses [section] shortcodes (blog-section-card divs already present),
 * skips legacy H2-splitting. Falls back to auto-wrapping for old plain-HTML posts.
 */
function processHTML(html: string): string {
  // Inject IDs into h2/h3 headings that don't already have one
  ...
  // If [section] shortcodes were used, the structure is already defined — done.
  if (out.includes('class="blog-section-card"')) return out;

  // Legacy fallback: split plain-HTML posts at every H2 boundary into cards.
  ...
  return (
    (preamble.trim() ? card(preamble, true) : "") +
...
export default function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const run = async () => {
      try {
        ...
        if (data.data.category) {
          const relRes = await fetch(`${API_BASE}/api/blogs?category=${encodeURIComponent(data.data.category)}&limit=4`);
          const relData = await relRes.json();
          if (relData.success)
            setRelated(relData.data.filter((p: BlogPost) => p.slug !== slug).slice(0, 3));
        }
  ...
  // Process HTML and extract TOC from the string — no DOM timing issues
  const processedHTML = useMemo(() => (post ? processHTML(parseShortcodes(post.content)) : ""), [post]);
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="flex flex-col items-center justify-center h-[60vh] gap-4 text-center px-4">
          ...
          <div>
            <h1 className="text-gray-900 text-2xl font-bold mb-2">Article not found</h1>
            <p className="text-gray-400 text-sm">This post doesn&apos;t exist or has been unpublished.</p>
          </div>
          <Link href="/blog" className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white font-semibold px-5 py-2.5 rounded-xl text-sm transition-colors">
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <aside className="space-y-5 lg:sticky lg:top-24 self-start">
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm">
              ...
              <div className="grid grid-cols-3 gap-1.5">
                {[
                  ...
                  <a
                    key={name}
                    href={url(typeof window !== "undefined" ? window.location.href : "")}
                    target="_blank"
                    rel="noopener noreferrer"
                    title={`Summarise with ${name}`}
            ...
            <div className="bg-black rounded-2xl p-6 text-center">
              ...
              <p className="text-white/45 text-xs leading-relaxed mb-4">
                Convert, migrate, backup and view emails — all in one offline tool.
              </p>
              <a
                href="/#download"
                className="block w-full bg-red-600 hover:bg-red-700 text-white text-xs font-bold py-2.5 rounded-xl transition-colors"
              >
                Free Download
              </a>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
              ...
              <button
                onClick={handleShare}
                className="w-full flex items-center justify-center gap-2 bg-gray-900 hover:bg-black text-white text-xs font-semibold py-2.5 rounded-xl transition-colors"
              >
                {copied
                  ? <><Check className="w-3.5 h-3.5 text-green-400" /> Link copied!</>
      ...
      {tocItems.length > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex flex-col items-center">
          ...
          {/* TOC panel — slides up */}
          <div
            className={`mb-3 w-72 sm:w-80 bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden transition-all duration-300 origin-bottom ${
              tocOpen ? "opacity-100 scale-100 translate-y-0 pointer-events-auto" : "opacity-0 scale-95 translate-y-4 pointer-events-none"
            }`}
          >
            {/* Header */}
            ...
            <nav className="max-h-72 overflow-y-auto py-2">
              {tocItems.map((item, i) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-2 text-left hover:bg-red-50 hover:text-red-600 transition-colors group ${
                    item.level === 3 ? "pl-8" : ""
                  }`}
                >
                  <span className="w-5 h-5 rounded-full bg-gray-100 group-hover:bg-red-100 text-gray-500 group-hover:text-red-600 text-[10px] font-bold flex items-center justify-center flex-shrink-0 transition-colors">
                    {i + 1}
                  </span>
                  <span className="text-gray-600 group-hover:text-red-600 text-xs leading-snug transition-colors line-clamp-2">
                    {item.text}
                  </span>
                </button>
              ))}
